...
def search():
    ...
    all_data = Data.query.all()
    all_names = Data.query.order_by(all_data.name).all()
    for i in all_names:
        if search_name == i.name:
            searchResult = all_data.filter(all_data.name.like('%searchForm.Name.data%'))
            return render_template("index.html",row = searchResult)
        else:
            flask("No Student Record Found By This Name")
